...
function convert(input, options) {
  ...
  options = options || {};
  doTrim = options.trim === false ? false : true;
...
function merge(template, data, options) {
  ...
  if (typeof template == 'string') {
    ...
    template = template.replace(PLACEHOLDER_RE, function (match, idx) {
...
function printf(template) {
  ...
  out = out.replace(/%[idjs%]/g, function (placeholder) {
    var replacement;
